...
<template>
    <label class="flex flex-col gap-2 text-sm font-medium text-slate-700">
        <span>{{ label }}</span>
        <textarea v-if="textarea" :value="modelValue" :class="textareaClass" :rows="rows" :placeholder="placeholder"
            @input="handleInput"></textarea>
        <input v-else :value="modelValue" :type="type" :class="inputClass" :placeholder="placeholder"
            @input="handleInput" />
    </label>
</template>
